( function ( blocks, blockEditor, components, element, i18n ) {
	...
	blocks.registerBlockType( 'parish-formation/question', {
		...
		attributes: {
			...
			fileAllowedExtensions: { type: 'string', default: 'pdf, doc, docx, jpg, jpeg, png' }, fileAllowedMimeTypes: { type: 'string', default: '' }, fileMaxSizeMb: { type: 'string', default: '5' }, fileMinimumCount: { type: 'integer', default: 1 }, fileMaximumCount: { type: 'integer', default: 1 }, fileSubmissionInstructions: { type: 'string', default: '' }
		},
		...
		edit: function ( props ) {
			...
			return el( 'div', blockEditor.useBlockProps( { className: 'pf-question-block' } ),
				el( InspectorControls, {},
					el( PanelBody, { title: __( 'Question Settings', 'parish-formation' ), initialOpen: true },
						el( SelectControl, { label: __( 'Question type', 'parish-formation' ), value: attrs.type, options: [
							...
							{ label: __( '— Instructor Reviewed —', 'parish-formation' ), value: '__review', disabled: true }, { label: __( 'Paragraph Response (Phase 3)', 'parish-formation' ), value: '__paragraph', disabled: true }, { label: __( 'File Upload', 'parish-formation' ), value: 'file_upload' },
						...
						], onChange: function ( value ) { if ( value.indexOf( '__' ) === 0 ) { return; } props.setAttributes( { type: value, graded: ! [ 'reflection', 'acknowledgement', 'rating_scale', 'yes_no' ].includes( value ), manualReview: [ 'reflection', 'file_upload' ].includes( value ) } ); } } ),
				...
				attrs.type === 'file_upload' && el( 'div', { className: 'pf-question-file-settings' },
					el( 'p', { className: 'description' }, __( 'Files are stored privately and always require staff review.', 'parish-formation' ) ),
					el( TextControl, { label: __( 'Allowed extensions', 'parish-formation' ), help: __( 'Separate extensions with commas. Executable and active web formats are always blocked.', 'parish-formation' ), value: attrs.fileAllowedExtensions, onChange: function ( value ) { props.setAttributes( { fileAllowedExtensions: value } ); } } ),
					el( TextareaControl, { label: __( 'Allowed MIME types (optional)', 'parish-formation' ), help: __( 'One MIME type per line. Leave blank to use WordPress MIME types for the allowed extensions.', 'parish-formation' ), value: attrs.fileAllowedMimeTypes, onChange: function ( value ) { props.setAttributes( { fileAllowedMimeTypes: value } ); } } ),
					el( TextControl, { label: __( 'Maximum size per file (MB)', 'parish-formation' ), type: 'number', min: 0.001, step: 0.1, value: attrs.fileMaxSizeMb, onChange: function ( value ) { props.setAttributes( { fileMaxSizeMb: value } ); } } ),
					el( TextControl, { label: __( 'Minimum number of files', 'parish-formation' ), type: 'number', min: 0, max: 10, value: attrs.fileMinimumCount, onChange: function ( value ) { props.setAttributes( { fileMinimumCount: Math.min( 10, Math.max( 0, parseInt( value, 10 ) || 0 ) ) } ); } } ),
					el( TextControl, { label: __( 'Maximum number of files', 'parish-formation' ), type: 'number', min: 1, max: 10, value: attrs.fileMaximumCount, onChange: function ( value ) { props.setAttributes( { fileMaximumCount: Math.min( 10, Math.max( 1, parseInt( value, 10 ) || 1 ) ) } ); } } ),
					attrs.fileMinimumCount > attrs.fileMaximumCount && el( Notice, { status: 'warning', isDismissible: false }, __( 'The minimum will be reduced to the maximum when saved.', 'parish-formation' ) ),
					el( TextareaControl, { label: __( 'Submission instructions (optional)', 'parish-formation' ), value: attrs.fileSubmissionInstructions, onChange: function ( value ) { props.setAttributes( { fileSubmissionInstructions: value } ); } } )
				),
