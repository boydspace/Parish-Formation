...
final class Parish_Formation_Account_Settings {
	...
	public static function render_page() {
		...
		<tr><th><?php esc_html_e( 'Passwordless login', 'parish-formation' ); ?></th><td><label><input type="checkbox" name="passwordless_login" value="1" <?php checked( $settings['passwordless_login'] ); ?>> <?php esc_html_e( 'Allow login by emailed magic link or one-time code', 'parish-formation' ); ?></label><p class="description"><?php esc_html_e( 'Credentials expire after 15 minutes and are invalidated immediately after use.', 'parish-formation' ); ?></p></td></tr>
		...
		<hr><h2><?php esc_html_e( 'Shortcodes', 'parish-formation' ); ?></h2><p><code>[parish_formation_login]</code> &nbsp; <code>[parish_formation_registration]</code> &nbsp; <code>[parish_formation_account_button]</code></p></div>
		<?php
	}
	...
	public static function handle_save() {
		...
		update_option( Parish_Formation_Account_Service::SETTINGS_OPTION, array( 'public_registration' => isset( $_POST['public_registration'] ) ? 1 : 0, 'passwordless_login' => isset( $_POST['passwordless_login'] ) ? 1 : 0, 'require_first_name' => isset( $_POST['require_first_name'] ) ? 1 : 0, 'require_last_name' => isset( $_POST['require_last_name'] ) ? 1 : 0, 'require_phone' => isset( $_POST['require_phone'] ) ? 1 : 0, 'password_mode' => isset( $_POST['password_mode'] ) && 'generated' === sanitize_key( $_POST['password_mode'] ) ? 'generated' : 'required', 'login_redirect' => isset( $_POST['login_redirect'] ) ? esc_url_raw( wp_unslash( $_POST['login_redirect'] ) ) : '', 'registration_redirect' => isset( $_POST['registration_redirect'] ) ? esc_url_raw( wp_unslash( $_POST['registration_redirect'] ) ) : '' ), false );
